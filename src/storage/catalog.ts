import {env} from '../config/env.js'; import {FirestoreCatalogRepository} from '../repositories/firestore-catalog.repository.js'; import {readStore as readLocal,getTitle as getLocal,upsertTitle as upsertLocal,findEpisode as findLocal,episodeIdsEqual,type LocalTitle} from './local-store.js'; import {normalizeTitle} from '../core/identity/normalize.js';
...
export async function findCatalogEpisode(id:string){if(!useFirestore())return findLocal(id);const direct=await firestore.getEpisode(id);if(direct)return direct;for(const t of await firestore.list())for(const season of t.seasons)for(const e of season.episodes)if(episodeIdsEqual(e.episodeId,id))return {title:t,episode:{...e,episodeId:String(e.episodeId)}};return null}
