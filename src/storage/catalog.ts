...
export async function findCatalogEpisode(id:string){if(!useFirestore())return findLocal(id);let direct:null|Awaited<ReturnType<FirestoreCatalogRepository['getEpisode']>>=null;try{direct=await firestore.getEpisode(id);}catch{direct=null;}if(direct)return direct;for(const t of await firestore.list())for(const season of t.seasons)for(const e of season.episodes)if(String(e.episodeId)===String(id))return {title:t,episode:e};return null}
