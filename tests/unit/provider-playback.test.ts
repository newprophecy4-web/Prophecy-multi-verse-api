...
const source=(overrides:Partial<LocalSource>={}):LocalSource=>({id:'s',providerId:'nasa-svs',sourceId:'1:0',format:'mp4',mimeType:'video/mp4',licenseStatus:'public-domain',licenseEvidence:'NASA',availability:'available',url:'https://svs.gsfc.nasa.gov/video.mp4',...overrides});
...
describe('provider playback hardening',()=>{
 ...
 it('rejects insecure, unavailable, unsupported, and mismatched sources',async()=>{expect(await validateSource(source({url:'http://svs.gsfc.nasa.gov/video.mp4'}))).toMatchObject({valid:false,reason:'INSECURE_URL'});expect(await validateSource(source({availability:'unavailable'}))).toMatchObject({valid:false,reason:'SOURCE_UNAVAILABLE'});expect(await validateSource(source({format:'mov',url:'https://svs.gsfc.nasa.gov/video.mov'}))).toMatchObject({valid:false,reason:'UNSUPPORTED_FORMAT'});expect(await validateSource(source({providerId:'nasa-svs',url:'https://archive.org/video.mp4'}))).toMatchObject({valid:false,reason:'PROVIDER_IDENTITY_MISMATCH'});});
 ...
 it('uses a bounded range request when HEAD is rejected',async()=>{const fetchSpy=vi.spyOn(globalThis,'fetch').mockResolvedValueOnce(new Response(null,{status:405})).mockResolvedValueOnce(new Response(null,{status:206,headers:{'content-type':'video/mp4'}}));expect(await validateSource(source())).toMatchObject({valid:true,contentType:'video/mp4'});expect(fetchSpy).toHaveBeenNthCalledWith(2,source().url,expect.objectContaining({headers:{range:'bytes=0-1'}}));});
